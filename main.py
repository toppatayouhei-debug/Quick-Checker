...
/* 青枠内のフォント設定（生物の正解・解説用） */
.stAlert div { font-family: serif !important; white-space: pre-wrap; }
...
# フィルタリング（全科目対応）
current_filter = "All"
nihonshi_titles = {"第1章": "歴史のはじまり", "第2章": "飛鳥時代", "第3章": "奈良時代", "第4章": "平安時代", "第5章": "院政と武士の躍進", "第6章": "武家政権の成立", "第7章": "武家社会の成長", "第8章": "近世の幕開け", "第9章": "幕藩体制の成立と展開", "第10章": "幕藩体制の動揺", "第11章": "近世から近代へ", "第12章": "近代国家の成立", "第13章": "近代国家の展開", "第14章": "近代の産業と生活"}
...
if subject == "システム英単語":
    ...
    options = ["すべてを表示"] + ([f"{c} {nihonshi_titles.get(c, '')}".strip() for c in raw_chaps] if "日本史" in subject else raw_chaps)
    sel_range = st.sidebar.radio("範囲を選択", options)
    current_filter = sel_range.split(" ")[0] if sel_range != "すべてを表示" else "すべて"
    df = raw_df if sel_range == "すべてを表示" else raw_df[raw_df["chapter"].astype(str).str.strip() == current_filter]
else:
    df = raw_df
...
if idx >= len(active_df):
    st.balloons(); st.success("全問終了！"); st.button("リセット", on_click=reset_quiz_engine); st.stop()
...
if subject == "暗唱例文集":
    ...
    if c_m2.button("🔵 ヒントはここ"): st.session_state.study_mode = "空欄補充"; st.rerun()
    disp = re.sub(r'\*\*(.*?)\*\*', "[ ____ ]", str(row["english"])) if st.session_state.study_mode == "空欄補充" else "（英文を思い出してください）"
    st.markdown(f'<div class="card orange-card">【日】{row["japanese"]}<hr>【英】{disp}</div>', unsafe_allow_html=True)
    if not st.session_state.answered:
        if st.button("答えを確認する"): st.session_state.answered = True; st.rerun()
    else:
        ans_h = re.sub(r'\*\*(.*?)\*\*', r'<span style="color:#e91e63; font-weight:800; border-bottom:2px solid;">\1</span>', str(row["english"]))
        st.markdown(f'<div class="exp-card">【正解】<br><span style="font-size:1.3rem; font-family:serif;">{ans_h}</span></div>', unsafe_allow_html=True)
...
# --- 日本史史料問題 ---
    ...
    correct_list = [a.strip() for a in str(row["answer"]).split("/") if a.strip()]
    user_inputs = []
...
# --- 生物一問一答（ここがご指定の修正箇所） ---
    ...
    if not st.session_state.answered:
        ...
        full_display = f"【正解】\n{row['answer']}"
        if pd.notna(row.get("explanation")): full_display += f"\n\n【解説】\n{row['explanation']}"
        st.success(full_display) # 青枠で正解＋解説を表示
        c1, c2 = st.columns(2)
        if c1.button("✅ 次へ"): st.session_state.idx += 1; st.session_state.answered = False; st.rerun()
        if c2.button("🔄 もう一度"): st.session_state.answered = False; st.rerun()
